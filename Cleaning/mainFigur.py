...
def setup_dask():
    ...
    cluster = LocalCluster(n_workers=18, threads_per_worker=4, memory_limit="100GB")
...
def main():
    ...
    os.makedirs('outputs/csv', exist_ok=True)    # ensure CSV output dir exists
    ...
    try:
        ...
        for i, df in enumerate(dataframes):
            ...
            df = df.reset_index(drop=True)
            result = df.compute()
            # write out intermediate after cleaning/filtering pipeline
            result.to_csv(f'outputs/csv/pipeline_stage_{i+1}.csv', index=False)
            computed_dfs.append(result)

        # merge step
        print("Merging dataframes...")
        merged = pd.concat(computed_dfs, ignore_index=True)
        merged.to_csv('outputs/csv/after_merge_without_outlier.csv', index=False)

        # trajectory reducer
        print("Applying trajectory_reducer...")
        reduced = trajectory_reducer(merged)
        reduced.to_csv('outputs/csv/after_trajectory_reducer_figur.csv', index=False)
        print(f"Reduced to {len(reduced)} rows from {len(merged)}")

        # now feed into poly_intersect
        print("Applying poly_intersect...")
        final_df = poly_intersect(reduced)
        print(f"Reduced to {len(final_df)} rows from {len(reduced)}")

        # final write
        final_df.to_csv('outputs/csv/cleaned_data_test.csv', index=False)
        print("All CSVs written under outputs/csv/")
