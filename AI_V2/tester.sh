...
#SBATCH --job-name=tester  # Name of your job
#SBATCH --output=tester%j.out     # Name of the output file
#SBATCH --error=tester%j.err      # Name of the error file
...
singularity exec --nv --bind $(pwd):/workspace containers/ai_container_l4.sif \
    bash -c "cd /workspace && torchrun --standalone --nnodes=1 --nproc_per_node=8 tester.py"
