...
#SBATCH --output=Transformer%j.out     # Name of the output file
#SBATCH --error=Transformer%j.err      # Name of the error file
...
singularity exec --nv --bind $(pwd):/workspace containers/ai_container_l4.sif \
    bash -c "cd /workspace && torchrun --standalone --nnodes=1 --nproc_per_node=1 Transformer.py"
