...
class AISTransformer(nn.Module):
    def __init__(self, feature_dim=5, hidden_dim=64, num_heads=2, num_layers=1, dropout=0.3, max_len=20):
        super().__init__()
...
def main():
    ...
    save_path = os.path.join("models", "TransformerWithGRU_year_5_v3.pt")
    ...
    data = np.load("data/processed_data_year_splits_v2.npz", allow_pickle=True)
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(eval_loader):
                ...
                for i in range(len(mmsi_values)):
                    ...
                    gap_length = haversine(input_lat[i][9], input_lon[i][9], input_lat[i][15], input_lon[i][15])
