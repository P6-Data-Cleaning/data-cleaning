...
class AISTransformer(nn.Module):
    def __init__(self, feature_dim=5, hidden_dim=64, num_heads=2, num_layers=1, dropout=0.3, max_len=100):
        super().__init__()
...
def main():
    ...
    save_path = os.path.join("models", "TransformerWithGRU_year_50_v3.pt")
    ...
    data = np.load("data/processed_data_year_splits_50_v2.npz", allow_pickle=True)
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(eval_loader):
                ...
                for i in range(len(mmsi_values)):
                    ...
                    gap_length = haversine(input_lat[i][24], input_lon[i][24], input_lat[i][75], input_lon[i][75])
